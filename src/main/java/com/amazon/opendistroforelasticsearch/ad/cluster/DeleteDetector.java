...
public class DeleteDetector {
    ...
    private void deleteDetectorResult(
        ...
        DeleteByQueryRequest
            deleteRequest =
            new DeleteByQueryRequest(AnomalyDetectionIndices.ALL_AD_RESULTS_INDEX_PATTERN)
                .setQuery(new BoolQueryBuilder().filter(QueryBuilders.termsQuery(AnomalyResult.DETECTOR_ID_FIELD, detectorID))
                    .filter(QueryBuilders.rangeQuery(AnomalyResult.END_TIME_FIELD).lte(deleteBeforeEpochMillis)
