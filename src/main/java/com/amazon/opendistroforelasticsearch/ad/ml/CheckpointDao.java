...
public class CheckpointDao {
    ...
    public void getModelCheckpoint(String modelId, ActionListener<Optional<String>> listener) {
        client.get(new GetRequest(indexName, DOC_TYPE, modelId), ActionListener.wrap(
            response -> listener.onResponse(processModelCheckpoint(response)), listener::onFailure));
    }
    ...
    public void deleteModelCheckpoint(String modelId, ActionListener<Void> listener) {
        client.delete(new DeleteRequest(indexName, DOC_TYPE, modelId), ActionListener.wrap(
           r -> listener.onResponse(null), listener::onFailure));
    }
}
