...
public class RestDeleteAnomalyDetectorAction extends BaseRestHandler {
    ...
    protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) throws IOException {
        ...
        WriteRequest.RefreshPolicy
            refreshPolicy =
            WriteRequest.RefreshPolicy.parse(request.param(REFRESH, WriteRequest.RefreshPolicy.IMMEDIATE.getValue()));
        ...
        return channel -> {
            if (channel.request().method() == RestRequest.Method.POST) {
                ...
                handler.getMonitorUsingDetector(
                    clusterService,
                    client,
                    detectorId,
                    channel,
    ...
    private void deleteAnomalyDetectorDoc(
        NodeClient client, String detectorId, RestChannel channel, WriteRequest.RefreshPolicy refreshPolicy) {
        logger.info("Delete anomaly detector {}", detectorId);
        DeleteRequest
            deleteRequest =
            new DeleteRequest(AnomalyDetector.ANOMALY_DETECTORS_INDEX, detectorId).setRefreshPolicy(refreshPolicy);
