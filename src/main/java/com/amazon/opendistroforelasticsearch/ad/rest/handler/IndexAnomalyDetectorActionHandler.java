...
public class IndexAnomalyDetectorActionHandler extends AbstractActionHandler {
    ...
    public void start() throws IOException {
        if (!anomalyDetectionIndices.doesAnomalyDetectorIndexExist()) {
            anomalyDetectionIndices.initAnomalyDetectorIndex(ActionListener
                .wrap(response -> onCreateMappingsResponse(response), exception -> onFailure(exception)));
        } else {
    ...
    private void onGetAnomalyDetectorResponse(GetResponse response) throws IOException {
        if (!response.isExists()) {
            XContentBuilder
                builder =
                channel.newErrorBuilder().startObject().field("Message", "AnomalyDetector is not found with id: " + detectorId).endObject();
    ...
    private void indexAnomalyDetector(String detectorId) throws IOException {
        IndexRequest
            indexRequest =
            new IndexRequest(ANOMALY_DETECTORS_INDEX).setRefreshPolicy(refreshPolicy)
                .source(anomalyDetector.toXContent(channel.newBuilder(), XCONTENT_WITH_TYPE)).setIfSeqNo(seqNo)
                .setIfPrimaryTerm(primaryTerm).timeout(requestTimeout);
        if (detectorId != null) {
    ...
    private ActionListener<IndexResponse> indexAnomalyDetectorResponse() {
        return new RestResponseListener<IndexResponse>(channel) {
            ...
            public RestResponse buildResponse(IndexResponse response) throws Exception {
                ...
                XContentBuilder
                    builder =
                    channel.newBuilder().startObject().field(RestHandlerUtils._ID, response.getId())
                        .field(RestHandlerUtils._VERSION, response.getVersion()).field(RestHandlerUtils._SEQ_NO, response.getSeqNo())
                        .field(RestHandlerUtils._PRIMARY_TERM, response.getPrimaryTerm()).field("anomaly_detector", anomalyDetector)
                        .endObject();
