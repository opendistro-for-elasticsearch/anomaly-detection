...
public class AnomalyDetectorRestApiIT extends AnomalyDetectorRestTestCase {
    ...
    public void testCreateAnomalyDetectorWithNotExistingIndices() throws Exception {
        ...
        TestHelpers.assertFailWith(ResponseException.class, "index_not_found_exception",
                () -> TestHelpers.makeRequest(client(), "POST", TestHelpers.AD_BASE_DETECTORS_URI, ImmutableMap.of(),
                        toHttpEntity(detector), null));
    }

    public void testCreateAnomalyDetectorWithEmptyIndices() throws Exception {
        AnomalyDetector detector = TestHelpers.randomAnomalyDetector(TestHelpers.randomUiMetadata(), null);
        TestHelpers.makeRequest(client(), "PUT", "/" + detector.getIndices().get(0), ImmutableMap.of(),
                toHttpEntity("{\"settings\":{\"number_of_shards\":1},\"mappings\":{\"properties\":" +
                        "{\"field1\":{\"type\":\"text\"}}}}"), null);

        TestHelpers.assertFailWith(ResponseException.class, "Can't create anomaly detector as no document found in indices",
                () -> TestHelpers.makeRequest(client(), "POST", TestHelpers.AD_BASE_DETECTORS_URI, ImmutableMap.of(),
                        toHttpEntity(detector), null));
    }
    ...
    public void testGetNotExistingAnomalyDetector() throws Exception {
        ...
        TestHelpers.assertFailWith(ResponseException.class, null,
                () -> getAnomalyDetector(randomAlphaOfLength(5)));
    }
    ...
    public void testUpdateAnomalyDetector() throws IOException {
        ...
        AnomalyDetector newDetector = new AnomalyDetector(detector.getDetectorId(), detector.getVersion(),
                detector.getName(), newDescription, detector.getTimeField(), detector.getIndices(),
                detector.getFeatureAttributes(),
                detector.getFilterQuery(), detector.getDetectionInterval(), detector.getWindowDelay(), detector.getUiMetadata(),
                detector.getSchemaVersion(), detector.getLastUpdateTime());

        Response updateResponse = TestHelpers.makeRequest(client(), "PUT",
                TestHelpers.AD_BASE_DETECTORS_URI + "/" + detector.getDetectorId() + "?refresh=true",
                ImmutableMap.of(), toHttpEntity(newDetector), null);
    ...
    public void testUpdateAnomalyDetectorWithNotExistingIndex() throws Exception {
        ...
        TestHelpers.assertFailWith(ResponseException.class, null, () -> TestHelpers.makeRequest(client(),
                "PUT", TestHelpers.AD_BASE_DETECTORS_URI + "/"
    ...
    public void testPreviewAnomalyDetector() throws IOException {
        ...
        AnomalyDetectorExecutionInput input = new AnomalyDetectorExecutionInput(detector.getDetectorId(),
                Instant.now().minusSeconds(60 * 10), Instant.now());
        Response response = TestHelpers.makeRequest(client(), "POST", TestHelpers.AD_BASE_PREVIEW_URI,
                ImmutableMap.of(),
                toHttpEntity(input), null);
    ...
    public void testPreviewAnomalyDetectorWhichNotExist() throws Exception {
        AnomalyDetector detector = createRandomAnomalyDetector(true, false);
        AnomalyDetectorExecutionInput input = new AnomalyDetectorExecutionInput(randomAlphaOfLength(5),
                Instant.now().minusSeconds(60 * 10), Instant.now());
        TestHelpers.assertFailWith(ResponseException.class, () -> TestHelpers.makeRequest(client(), "POST", TestHelpers.AD_BASE_PREVIEW_URI,
                ImmutableMap.of(),
                toHttpEntity(input), null));
    }

    public void testExecuteAnomalyDetectorWithNullDetectorId() throws Exception {
        AnomalyDetectorExecutionInput input = new AnomalyDetectorExecutionInput(null,
                Instant.now().minusSeconds(60 * 10), Instant.now());
        TestHelpers.assertFailWith(ResponseException.class, () -> TestHelpers.makeRequest(client(), "POST", TestHelpers.AD_BASE_PREVIEW_URI,
                ImmutableMap.of(),
                toHttpEntity(input), null));
    ...
    public void testSearchAnomalyResult() throws IOException {
        ...
        Response response = TestHelpers.makeRequest(client(), "POST", "/.opendistro-anomaly-results/_doc/" + UUIDs.base64UUID(),
                ImmutableMap.of(),
                toHttpEntity(anomalyResult), null);
        assertEquals("Post anomaly result failed", RestStatus.CREATED, restStatus(response));

        SearchSourceBuilder search = (new SearchSourceBuilder()).query(QueryBuilders.termQuery("detector_id",
                anomalyResult.getDetectorId()));
        Response searchResponse = TestHelpers.makeRequest(client(), "POST", TestHelpers.AD_BASE_RESULT_URI + "/_search",
                ImmutableMap.of(),
                new NStringEntity(search.toString(), ContentType.APPLICATION_JSON), null);
        ...
        Response searchAllResponse = TestHelpers.makeRequest(client(), "POST", TestHelpers.AD_BASE_RESULT_URI + "/_search",
                ImmutableMap.of(),
                new NStringEntity(searchAll.toString(), ContentType.APPLICATION_JSON), null);
    ...
    public void testDeleteAnomalyDetector() throws IOException {
        AnomalyDetector detector = createRandomAnomalyDetector(true, false);
        Response response = TestHelpers.makeRequest(client(), "DELETE", TestHelpers.AD_BASE_DETECTORS_URI + "/"
                + detector.getDetectorId(), ImmutableMap.of(), "", null);
    ...
    public void testDeleteAnomalyDetectorWhichNotExist() throws Exception {
        TestHelpers.assertFailWith(ResponseException.class, () -> TestHelpers.makeRequest(client(),
                "DELETE", TestHelpers.AD_BASE_DETECTORS_URI + "/"
                + randomAlphaOfLength(5), ImmutableMap.of(), "", null));
    }

    public void testDeleteAnomalyDetectorWithNoMonitor() throws Exception {
        TestHelpers.makeRequest(client(), "PUT", "/" + MONITOR_INDEX_NAME,
                ImmutableMap.of(),
                MONITOR_INDEX_MAPPING, null);
    ...
    public void testDeleteAnomalyDetectorWithUsedByMonitor() throws Exception {
        ...
        Response createResponse = TestHelpers.makeRequest(client(), "POST", "/" + MONITOR_INDEX_NAME + "/_doc/" + monitorId,
                ...
                ("{\"monitor\":{\"type\":\"monitor\",\"name\":\"test-monitor-with-ad\",\"enabled\":true,\"schedule\":" +
                        ...
                        "\"painless\"}},\"actions\":[]}]}}").replace("DETECTOR_ID_PLACEHOLDER",
                        detector.getDetectorId()), null);

        assertEquals("Post anomaly result failed", RestStatus.CREATED, restStatus(createResponse));

        TestHelpers.assertFailWith(ResponseException.class, "Detector is used by monitor: " + monitorId,
                () -> TestHelpers.makeRequest(client(), "DELETE", TestHelpers.AD_BASE_DETECTORS_URI + "/"
                        + detector.getDetectorId(), ImmutableMap.of(), "", null));
    }
}
